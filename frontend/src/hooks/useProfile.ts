...
function getErrorMessage(error: unknown): string {
  ...
  if (e?.response?.data?.detail) {
    ...
    if (Array.isArray(detail)) {
      return detail
        .map((err: any) => `${err?.loc?.[1] || err?.loc?.[0] || "field"}: ${err?.msg || "invalid"}`)
        .join(", ");
    }
...
export const useApiKeys = () => {
  ...
  const createApiKey = async (request: CreateApiKeyRequest): Promise<CreateApiKeyResponse | null> => {
    ...
    try {
      ...
      const fullKey = `dhc_${Math.random().toString(36).substring(2, 15)}${Math.random().toString(36).substring(2, 15)}`;
  ...
  const rotateApiKey = async (id: string): Promise<CreateApiKeyResponse | null> => {
    ...
    try {
      ...
      setApiKeys((prev) =>
        prev.map((k) =>
          k.id === id ? { ...k, prefix: rotatedKey.prefix, created_at: rotatedKey.created_at } : k
        )
      );
