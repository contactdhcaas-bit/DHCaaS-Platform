...
const CreatePolicyModal: React.FC<CreatePolicyModalProps> = ({
  ...
  const ruleTypes = [
    { value: 'min_quality_score', label: 'Minimum Quality Score', placeholder: 'e.g., 80' },
    { value: 'max_pii_rows', label: 'Max PII Rows', placeholder: 'e.g., 0 (no PII allowed)' },
    { value: 'max_duplicates', label: 'Max Duplicate Records', placeholder: 'e.g., 5' },
    { value: 'max_missing_percentage', label: 'Max Missing Data %', placeholder: 'e.g., 10' },
    { value: 'forbidden_columns', label: 'Forbidden Columns', placeholder: 'e.g., 0' },
    { value: 'require_encryption', label: 'Require Encryption', placeholder: 'e.g., 1' },
  ];
  ...
  const severityLevels = [
    ...
    { value: 'warning', label: 'Warning', color: 'text-blue-500' },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMsg = err.response?.data?.detail || err.response?.data?.message || 'Failed to create policy';
      setError(errorMsg);
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g., High Quality Data Policy"
                      required
                      disabled={loading}
