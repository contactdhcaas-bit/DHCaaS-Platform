...
async def register(
    ...
    """
    try:
        existing_user = await db.dhcaas.users.find_one({"email": user_data.email})
        ...
        result = await db.dhcaas.users.insert_one(user_doc)
...
async def login(
    ...
    try:
        ...
        user = await db.dhcaas.users.find_one({"email": form_data.username})
        ...
        # Update last login
        await db.dhcaas.users.update_one(
            {"_id": user["_id"]},
...
async def get_current_user_info(
    ...
    try:
        ...
        user = await db.dhcaas.users.find_one({"_id": ObjectId(current_user["user_id"])})
